...
<div class="page-head">
 ...
 <div class="editor-actions"><a class="button" href="<?=e(app_url('/studio/content'))?>">Назад</a><?php if(!$isNew):?><a class="button" target="_blank" href="<?=e(app_url('/studio/content/'.(int)$entry['id'].'/preview'))?>">Предпросмотр</a><?php endif;?><button class="button primary" type="submit">Сохранить</button></div>
</div>
...
<div class="editor-layout">
 <div class="editor-main">
  <section class="editor-card editor-card--basic">
   ...
   <div class="form-grid"><div class="field"><label>Тип</label><select name="type"><option value="post" <?=$type==='post'?'selected':''?>>Публикация</option><option value="page" <?=$type==='page'?'selected':''?>>Страница</option><option value="portfolio" <?=$type==='portfolio'?'selected':''?>>Портфолио</option></select></div><div class="field"><label>Короткий анонс</label><input name="excerpt" maxlength="2000" value="<?=e((string)($entry['excerpt']??''))?>" placeholder="Текст для карточки в ленте"></div></div>
  </section>

  <section class="editor-card classic-editor-card" data-classic-editor-shell data-classic-id="classic-<?=((int)($entry['id']??0)?:'new')?>">
...
<div class="classic-editor-preview" data-classic-preview hidden><div class="classic-editor-preview__dialog"><header class="classic-editor-preview__head"><h2>Предпросмотр</h2><button type="button" data-close-classic-preview>×</button></header><iframe title="Предпросмотр"></iframe></div></div>
