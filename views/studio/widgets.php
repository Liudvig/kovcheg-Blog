...
$renderFields=static function(array $definition,array $settings)use($menus):void{
    foreach((array)($definition['fields']??[]) as $key=>$field){
        ...
        <?php }elseif($type==='menu'){?>
            <label><span><?=e($label)?></span><select name="<?=e($key)?>"><option value="0">Меню по назначенной позиции</option><?php foreach($menus as $menu):?><option value="<?=(int)$menu['id']?>" <?=(int)$value===(int)$menu['id']?'selected':''?>><?=e($menu['name'])?></option><?php endforeach;?></select></label>
...
<article class="widget-card <?=$instance['is_enabled']?'':'is-disabled'?> <?=$available?'':'is-missing'?>" draggable="true" data-widget-id="<?=(int)$instance['id']?>">
  ...
  <div class="widget-card__move"><button type="button" data-widget-up aria-label="Выше">↑</button><button type="button" data-widget-down aria-label="Ниже">↓</button><button type="button" data-widget-pool>Убрать с сайта</button></div>
  ...
  <footer class="widget-card__actions">
    ...
    <form method="post" data-confirm="Удалить виджет?" action="<?=e(app_url('/studio/widgets/'.(int)$instance['id'].'/delete'))?>"><?=csrf_field()?><input type="hidden" name="layout_id" value="<?=(int)$currentLayout['id']?>"><button class="button danger small">Удалить</button></form>
  </footer>
</article>
<?php };
...
<section class="widget-zone matrix-zone <?=e($visualClass)?> <?=$items?'has-widgets':''?>" data-zone-card data-zone-id="<?=e($zoneId)?>">
  ...
  <div class="widget-zone__items" data-widget-zone="<?=e($zoneId)?>">
    ...
    <p class="widget-zone__empty">Перетащите сюда виджет</p>
  </div>
</section>
<?php };
?>

<div class="studio-page-head widget-page-head"><div><span class="eyebrow">KOVCHEG CMS <?=e(APP_VERSION)?></span><h1>Виджеты и зоны</h1><p>Создайте нужные блоки, настройте их и перетащите в шапку, колонки, содержимое или подвал. Ненужные виджеты можно отключить или убрать с сайта.</p></div><div class="studio-actions"><a class="button" href="<?=e(app_url('/studio/menus'))?>">Меню</a><a class="button" href="<?=e(app_url('/'))?>" target="_blank" rel="noopener">Открыть сайт</a></div></div>
...
<section class="studio-card widget-layout-toolbar">
  ...
  <div class="widget-layout-help">Рубрики, поиск, регистрация и другие блоки не выводятся сами — только когда виджет включён и размещён.</div>
...
<div class="widget-builder-shell widget-builder-shell--matrix">
  <aside class="studio-card widget-catalog widget-catalog--accordion">
    <details class="widget-library-group" open>
      <summary><span><b>Добавить виджет</b><small><?=count($widgetTypes)?> типов</small></span><i>⌄</i></summary>
    ...
    <details class="widget-library-group">
      <summary><span><b>История расположения</b><small><?=count($revisions)?> ревиз.</small></span><i>⌄</i></summary>
  ...
  <main class="widget-page-blueprint matrix-blueprint" aria-label="Схема зон сайта">
    <section class="matrix-row matrix-row--full matrix-preheader"><div class="matrix-row__title">НАД ШАПКОЙ</div><?php $renderZone('matrix.preheader','matrix-zone--full');?></section>
    ...
    <section class="matrix-row matrix-row--full matrix-postheader"><div class="matrix-row__title">ПОД ШАПКОЙ</div><?php $renderZone('matrix.postheader','matrix-zone--full');?></section>
    <section class="matrix-row matrix-row--full matrix-banner"><div class="matrix-row__title">ВЕРХНЯЯ ПОЛОСА</div><?php $renderZone('matrix.banner.top','matrix-zone--banner');?></section>

    <section class="matrix-columns">
      <aside class="matrix-sidebar matrix-sidebar--left"><div class="matrix-row__title">ЛЕВАЯ КОЛОНКА</div><div class="matrix-sidebar__grid"><?php for($i=1;$i<=4;$i++)$renderZone('matrix.left.'.$i,'matrix-zone--sidebar');?></div></aside>

      <section class="matrix-center"><div class="matrix-row__title">ЦЕНТРАЛЬНАЯ ОБЛАСТЬ</div><div class="matrix-center__grid"><?php for($i=1;$i<=4;$i++)$renderZone('matrix.center.'.$i,'matrix-zone--center');?><div class="matrix-content-lock"><span>ОСНОВНОЕ СОДЕРЖИМОЕ</span><b>Главная, запись или страница</b><small>Системный блок содержимого</small></div><?php for($i=5;$i<=12;$i++)$renderZone('matrix.center.'.$i,'matrix-zone--center');?></div></section>

      <aside class="matrix-sidebar matrix-sidebar--right"><div class="matrix-row__title">ПРАВАЯ КОЛОНКА</div><div class="matrix-sidebar__grid"><?php for($i=1;$i<=4;$i++)$renderZone('matrix.right.'.$i,'matrix-zone--sidebar');?></div></aside>
    </section>

    <section class="matrix-row matrix-row--full matrix-banner"><div class="matrix-row__title">НИЖНЯЯ ПОЛОСА</div><?php $renderZone('matrix.banner.bottom','matrix-zone--banner');?></section>

    <section class="matrix-row matrix-footer"><div class="matrix-row__title">ПОДВАЛ · 8 ЯЧЕЕК</div><div class="matrix-footer__grid"><?php for($i=1;$i<=8;$i++)$renderZone('matrix.footer.'.$i,'matrix-zone--footer');?></div><div class="matrix-copyright-lock"><b>© <?=date('Y')?> Ланцет Семён Борисович</b><span>KOVCHEG CMS · Все права защищены</span></div></section>
  </main>
</div>
