...
## 3.5.0 — KOVCHEG Portal

Добавлена самостоятельная тема новостного сайта, журнала и информационного портала поверх Layout & Widget Engine.

### Гибкая структура

- одна колонка при пустых боковых зонах;
- центральная область и левая колонка;
- центральная область и правая колонка;
- полноценная трёхколоночная схема;
- автоматическая перестройка колонок на планшетах и телефонах;
- отдельные зоны шапки, содержимого и подвала;
- пустые зоны не оставляют белых колонок.

### Новостное оформление

- ведущая публикация на главной;
- сетка следующих свежих материалов;
- дополнительная лента публикаций;
- блок проектов и портфолио;
- обложки через защищённый медиамаршрут;
- новый архив блога, рубрик, тегов, поиска и портфолио;
- статьи, страницы, авторы, реакции и комментарии совместимы со стабильным контентным шаблоном.

### Полная управляемость

- меню размещается только виджетом;
- разрешены несколько меню в разных зонах;
- поиск не встроен в шапку или архив и может отсутствовать полностью;
- левая и правая колонки наполняются виджетами и модулями;
- Module First виджеты автоматически работают в Portal;
- тема не содержит привязки к конкретным плагинам.

### SEO и качество

- canonical, robots, Open Graph, RSS и sitemap остаются системными и не зависят от темы;
- тема регистрируется идемпотентной миграцией;
- отдельный CI включает Portal и проверяет главную, блог и портфолио;
- проверяется отсутствие жёстко встроенной формы поиска в архиве;
- версия приложения поднята до 3.5.0.
