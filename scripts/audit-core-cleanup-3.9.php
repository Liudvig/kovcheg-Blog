...
$expect(!str_contains($studioMigration,'INSERT IGNORE INTO content_categories'),'Fresh Studio migration не должна создавать демонстрационные рубрики.');
$expect(!str_contains($studioMigration,"'Портфолио','/portfolio'"),'Fresh Studio migration не должна создавать пункт Портфолио.');
$expect(!str_contains($studioMigration,"'Блог','/blog'"),'Fresh Studio migration не должна создавать обязательный пункт Блог.');
$expect(!str_contains($studioMigration,'portfolio_description'),'Fresh Studio migration не должна создавать portfolio settings.');
$expect(str_contains($studioMigration,"'blog_tagline','Записи · страницы · рубрики'"),'Fresh Studio migration должна использовать нейтральный CMS tagline.');
