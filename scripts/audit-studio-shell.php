...
$bootstrap=$read('app/bootstrap.php');$core=$read('app/Core.php');$layout=$read('views/layout.php');$login=$read('views/login.php');$css=$read('assets/css/blog-admin-shell.css');$js=$read('assets/js/blog-admin-shell.js');
$required=[
 [preg_match("/const APP_VERSION = '([0-9.]+)';/",$bootstrap,$version)===1&&version_compare($version[1],'3.4.1','>='),'Версия приложения должна быть 3.4.1 или новее.'],
 ...
 [str_contains($layout,'blog-admin-shell.css'),'Layout не подключает отдельные стили старой системной админки.'],
 [str_contains($layout,'blog-admin-shell.js'),'Layout не подключает логику старой системной админки.'],
 ...
 [str_contains($css,'grid-template-columns:280px minmax(0,1fr)'),'Не найдена фиксированная двухколоночная сетка системной админки.'],
 ...
 [str_contains($css,'.admin-content>.admin-footer'),'Не найдены стили подвала системной админки.'],
 [str_contains($js,"['KOVCHEG CMS', 'KOVCHEG Blog']"),'Не найдена очистка старого названия в админке.'],
];
foreach($required as[$ok,$message])if(!$ok)$errors[]=$message;
if($errors){fwrite(STDERR,"Studio shell audit failed:\n- ".implode("\n- ",$errors)."\n");exit(1);}echo "Studio shell audit passed.\n";